/**
 * SafeMap Interactive Map Demo - Production Ready
 * Clean implementation without debug code for production deployment
 */
...
/**
 * Creates custom incident markers with severity-based styling
 * @param severity - Incident severity level ('high', 'medium', 'low')
 * @returns Leaflet DivIcon for the incident marker
 */
const createCustomIcon = (severity: string) => {
  ...
  switch (severity) {
    case "high":
      ...
      break;
    case "medium":
      color = "#f59e0b"; // Orange for medium severity
      emoji = "⚠️";
      break;
    case "low":
      color = "#eab308"; // Yellow for low severity
      emoji = "🔸";
      break;
  }
...
/**
 * Creates user location marker icon
 * @returns Leaflet DivIcon for user location
 */
...
// Mock incident data for Lima, Peru with realistic coordinates
const limaIncidents = [
  {
    id: 1,
    lat: -12.0464,
...
type ViewMode = "incidents" | "heatmap" | "routes";
type MapProvider = "openstreet" | "satellite" | "dark";

/**
 * Main MapDemo component for SafeMap landing page
 */
export const MapDemoClean: React.FC = () => {
  ...
  const [mapProvider, setMapProvider] = useState<MapProvider>("openstreet");
  const [showRealTime, setShowRealTime] = useState(true);
  const [showProviderInfo, setShowProviderInfo] = useState(false);
  const [userLocation, setUserLocation] = useState<UserLocation | null>(null);

  /**
   * Simulates user location in Lima for demo purposes
   */
  ...
  /**
   * Returns localized severity text
   */
  ...
  return (
    <div className="w-full h-full min-h-[400px] relative bg-gray-100 rounded-lg overflow-hidden border border-gray-300">
        ...
        <div className="bg-white/95 backdrop-blur-sm rounded-lg p-2 shadow-lg">
          <div className="flex items-center space-x-1">
            {Object.entries(mapProviders).map(([key, provider]) => (
              <button
                key={key}
                onClick={() => setMapProvider(key as MapProvider)}
      ...
      {/* Routes panel */}
      {viewMode === "routes" && (
        <div className="absolute bottom-24 right-4 bg-white/95 backdrop-blur-sm rounded-lg p-4 shadow-lg max-w-xs z-[1000]">
